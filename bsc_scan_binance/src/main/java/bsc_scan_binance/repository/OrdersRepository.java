...
public interface OrdersRepository extends JpaRepository<Orders, String> {
    ...
    @Query(value = "SELECT * FROM public.orders det where (det.gecko_id like '%HOUR')                        and det.gecko_id = (select REPLACE(mst.gecko_id, 'DAY', 'HOUR')   from orders mst where det.trend=mst.trend and mst.gecko_id=REPLACE(det.gecko_id, 'HOUR'  , 'DAY')) ORDER BY det.trend, det.gecko_id ", nativeQuery = true)
