...
public interface OrdersRepository extends JpaRepository<Orders, String> {
    ...
    @Query(value = "SELECT m.* FROM orders m WHERE (gecko_id like '%HOUR%') AND (TO_CHAR(created_at, 'YYYY-MM-DD HH24:mm') < TO_CHAR(NOW() - interval '2 hours', 'YYYY-MM-DD HH24:mm')) ", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM public.orders det where (det.gecko_id like '%HOUR')   and (det.note <> '') and det.gecko_id = (select REPLACE(mst.gecko_id, 'DAY', 'HOUR')   from orders mst where det.trend=mst.trend and mst.gecko_id=REPLACE(det.gecko_id, 'HOUR'  , 'DAY')) ORDER BY det.trend, det.gecko_id ", nativeQuery = true)
    public List<Orders> getTrend_H1List();
}
